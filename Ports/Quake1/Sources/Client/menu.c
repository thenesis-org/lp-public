...
static void M_Keys_Key(int key, int keyUnmodified);
...
void M_ScanSaves()
{
	...
	for (i = 0; i < MAX_SAVEGAMES; i++)
	{
		...
		loadable[i] = false;
		sprintf(name, "%s/s%i.sav", com_writableGamedir, i);
		f = fopen(name, "rb");
		if (!f)
			continue;
		fscanf(f, "%i\n", &version);
...
static void M_Keys_Key(int k, int keyUnmodified)
{
	int keys[2];
	...
	{
		S_LocalSound("misc/menu1.wav");
		if (keyUnmodified > 0 && keyUnmodified != K_ESCAPE)
		{
            char cmd[1024];
			snprintf(cmd, sizeof(cmd), "bind \"%s\" \"%s\"\n", Key_KeynumToString(keyUnmodified), bindnames[keys_cursor][0]);
            cmd[1023] = 0;
			Cbuf_InsertText(cmd);
		}
