...
float V_CalcBob()
{
	...
	if (cycle < cl_bobup.value)
		...
		cycle = Q_PI + Q_PI * (cycle - cl_bobup.value) / (1.0 - cl_bobup.value);
...
void BuildGammaTable(float g)
{
    ...
    else
    {
        for (int i = 0; i < 256; i++)
        {
            int inf = 255 * powf((i + 0.5f) / 255.5f, g) + 0.5f;
            if (inf < 0)
                inf = 0;
            if (inf > 255)
                inf = 255;
...
void V_CalcBlend()
{
    ...
    {
        float globalPercent = gl_cshiftpercent.value;
        for (int j = 0; j < NUM_CSHIFTS; j++)
        {
            cshift_t *cshift = &cl.cshifts[j];
            float a2 = (cshift->percent * globalPercent) * 1.0f / (100.0f * 255.0f);
            if (a2 <= 0.0f)
                continue;
            a = a + a2 * (1.0f - a);
            a2 = a2 / a;
            r = r * (1.0f - a2) + cshift->destcolor[0] * a2;
            g = g * (1.0f - a2) + cshift->destcolor[1] * a2;
            b = b * (1.0f - a2) + cshift->destcolor[2] * a2;
        }
    }
    
	v_blend[0] = r * (1.0f / 255.0f);
	v_blend[1] = g * (1.0f / 255.0f);
	v_blend[2] = b * (1.0f / 255.0f);
	if (a > 1)
		a = 1;
	if (a < 0)
		a = 0;
...
void V_UpdatePalette()
{
    ...
    // The modified palette is only used when gamma has changed.
    // Textures are not modified instantly since we use 32-bit rendering. The palette is applied when the textures are loaded (when changing level).
    
   	byte pal[768];
	byte *basepal = host_basepal, *newpal = pal;
