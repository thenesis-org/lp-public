...
void BuildTris(aliashdr_t *pheader)
{
	...
	for (i = 0; i < pheader->numtris; i++)
	{
		...
		for (j = 0; j < bestlen + 2; j++)
		{
            ...
            float *command = (float *)&commands[numcommands];
            command[0] = s;
            command[1] = t;
            numcommands += 2;
		}
...
void GL_MakeAliasModelDisplayLists(model_t *m, aliashdr_t *pheader)
{
//	aliasmodel = m;
    ...
    #endif
	{
		...
		BuildTris(pheader); // trifans or lists
		...
		sprintf(fullpath, "%s/%s", com_writableGamedir, cache);
		f = fopen(fullpath, "wb");
		if (f)
		{
