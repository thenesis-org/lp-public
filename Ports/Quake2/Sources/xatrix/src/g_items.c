...
qboolean Add_Ammo(edict_t *ent, gitem_t *item, int count)
{
	...
	if (item->tag == AMMO_MAGSLUG)
	{
		max = ent->client->pers.max_magslug;
	}
	else
	if (item->tag == AMMO_TRAP)
	{
...
/*
 * Sets the clipping size and plants the object on the floorf.
 * Items can't be immediately dropped to floorf, because they might
