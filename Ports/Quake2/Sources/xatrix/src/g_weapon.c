...
void bfg_touch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	/* core explosion - prevents firing it into the wall/floorf */
	if (other->takedamage)
	{
		T_Damage(other, self, self->owner, self->velocity, self->s.origin,
...
void fire_heat(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius, int radius_damage)
{
	...
	heat->nextthink = level.time + 0.1f;
	...
	if (ent->s.frame > 4)
	{
		if (ent->s.frame == 5)
		{
			...
			for (i = 0; i < 3; i++)
			{
				...
				RotatePointAroundVector(vec, up, right, ((360.0f / 3) * i) + ent->delay);
				...
				best->nextthink = level.time + 0.1f;
...
void fire_trap(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius, qboolean held)
{
	...
	trap->owner = self;
	trap->nextthink = level.time + 1.0f;
