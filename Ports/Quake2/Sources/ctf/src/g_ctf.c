...
static void CTFSetIDView(edict_t *ent)
{
	...
	if (level.time - ent->client->resp.lastidtime < 0.25f)
	{
		return;
	}
	...
	if ((tr.fraction < 1) && tr.ent && tr.ent->client)
	{
		...
		if (tr.ent->client->resp.ctf_team == CTF_TEAM2)
		{
	...
	if (bd > 0.90f)
	{
		...
		if (best->client->resp.ctf_team == CTF_TEAM2)
		{
			ent->client->ps.stats[STAT_CTF_ID_VIEW_COLOR] = imageindex_sbfctf2;
		}
	}
...
void SetCTFStats(edict_t *ent)
{
	...
	if (level.intermissiontime && (level.framenum & 8)) /* blink 1/8th second */
	{
		...
		if (ctfgame.team2 > ctfgame.team1)
		{
			ent->client->ps.stats[STAT_CTF_TEAM2_HEADER] = 0;
		}
		else
		if (ctfgame.total1 > ctfgame.total2)      /* frag tie breaker */
		{
			ent->client->ps.stats[STAT_CTF_TEAM1_HEADER] = 0;
		}
		else
		if (ctfgame.total2 > ctfgame.total1)
		{
			ent->client->ps.stats[STAT_CTF_TEAM2_HEADER] = 0;
		}
		else /* tie game! */
...
void CTFScoreboardMessage(edict_t *ent, edict_t *killer)
{
	...
	*string = 0;
	...
	for (i = 0; i < 16; i++)
	{
		...
		*entry = 0;
		...
		if (i < total[0])
		{
			...
			sprintf(entry + Q_strlen(entry), "ctf 0 %d %d %d %d ",
				42 + i * 8, sorted[0][i], cl->resp.score,
				cl->ping > 999 ? 999 : cl->ping);

			if (cl_ent->client->pers.inventory[ITEM_INDEX(flag2_item)])
			{
				sprintf(entry + Q_strlen(entry), "xv 56 yv %d picn sbfctf2 ",
					42 + i * 8);
			}

			if (maxsize - len > (int)Q_strlen(entry))
			{
				strcat(string, entry);
				len = Q_strlen(string);
				last[0] = i;
		...
		if (i < total[1])
		{
			...
			sprintf(entry + Q_strlen(entry), "ctf 160 %d %d %d %d ",
				42 + i * 8, sorted[1][i], cl->resp.score,
				cl->ping > 999 ? 999 : cl->ping);

			if (cl_ent->client->pers.inventory[ITEM_INDEX(flag1_item)])
			{
				sprintf(entry + Q_strlen(entry), "xv 216 yv %d picn sbfctf1 ",
					42 + i * 8);
			}
	...
	{
		sprintf(string + Q_strlen(string), "xv 8 yv %d string \"..and %d more\" ",
			42 + (last[0] + 1) * 8, total[0] - last[0] - 1);
	}

	if (total[1] - last[1] > 1) /* couldn't fit everyone */
	{
		sprintf(string + Q_strlen(
...
static void CTFSay_Team_Armor(edict_t *who, char *buf)
{
	...
	*buf = 0;
	...
	if (power_armor_type)
	{
		...
		if (cells)
		{
			sprintf(buf + Q_strlen(buf), "%s with %i cells ",
	...
	if (index)
	{
		...
		if (item)
		{
			...
			sprintf(buf + Q_strlen(buf), "%i units of %s",
...
static void CTFSay_Team_Sight(edict_t *who, char *buf)
{
	...
	*s = *s2 = 0;
	...
	for (i = 1; i <= maxclients->value; i++)
	{
		...
		if (*s2)
		{
			if (Q_strlen(s) + Q_strlen(s2) + 3 < (int)sizeof(s))
			{
				if (n)
				{
					strcat(s, ", ");
	...
	if (*s2)
	{
		if (Q_strlen(s) + Q_strlen(s2) + 6 < (int)sizeof(s))
		{
			if (n)
			{
				strcat(s, " and ");
...
void CTFSay_Team(edict_t *who, char *msg)
{
	...
	if (*msg == '\"')
	{
		msg[Q_strlen(msg) - 1] = 0;
		msg++;
	}

	for (p = outmsg; *msg && (p - outmsg) < (int)sizeof(outmsg) - 2; msg++)
	{
		if (*msg == '%')
		{
			switch (*++msg)
			...
			case 'L':
				...
				if (Q_strlen(buf) + (p - outmsg) < (int)sizeof(outmsg) - 2)
				{
					strcpy(p, buf);
					p += Q_strlen(buf);
				}
...
void CTFStartMatch(void)
{
	...
	for (i = 1; i <= maxclients->value; i++)
	{
		...
		if (ent->client->resp.ctf_team != CTF_NOTEAM)
		{
			...
			ent->client->respawn_time = level.time + 1.0f + ((rand() % 30) / 10.0f);
...
int CTFUpdateJoinMenu(edict_t *ent)
{
	...
	if (ctf_forcejoin->string && *ctf_forcejoin->string)
	{
		...
		if (Q_stricmp(ctf_forcejoin->string, "blue") == 0)
		{
...
/*
 * QUAKED trigger_teleport (0.5f 0.5f 0.5f) ?
...
/*
 * QUAKED info_teleport_destination (0.5f 0.5f 0.5f) (-16 -16 -24) (16 16 32)
...
void CTFOpenAdminMenu(edict_t *ent)
{
	...
	if ((ctfgame.match == MATCH_GAME) || (ctfgame.match == MATCH_PREGAME))
	{
		adminmenu[3].text = "Cancel match";
		adminmenu[3].SelectFunc = CTFAdmin_MatchSet;
	}
	else
	if ((ctfgame.match == MATCH_NONE) && competition->value)
	{
	...
	if (ctfgame.match == MATCH_SETUP)
	{
		for (i = 1; i <= maxclients->value; i++)
		{
			    ...
			    (e2->client->resp.ctf_team != CTF_NOTEAM))
			{
				...
				if (Q_strlen(text) + Q_strlen(st) < (int)sizeof(text) - 50)
				{
					strcat(text, st);
				}
			}
...
void CTFWarp(edict_t *ent)
{
	...
	mlist = Q_strdup(warp_list->string);
