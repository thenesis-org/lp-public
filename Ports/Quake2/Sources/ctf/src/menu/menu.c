...
pmenuhnd_t* PMenu_Open(edict_t *ent, pmenu_t *entries, int cur, int num, void *arg)
{
	...
	for (i = 0; i < num; i++)
	{
		if (entries[i].text)
		{
			hnd->entries[i].text = Q_strdup(entries[i].text);
		}
	}
...
void PMenu_Do_Update(edict_t *ent)
{
	...
	for (i = 0, p = hnd->entries; i < hnd->num; i++, p++)
	{
		...
		sprintf(string + Q_strlen(string), "yv %d ", 32 + i * 8);

		if (p->align == PMENU_ALIGN_CENTER)
		{
			x = 196 / 2 - Q_strlen(t) * 4 + 64;
		}
		else
		if (p->align == PMENU_ALIGN_RIGHT)
		{
			x = 64 + (196 - Q_strlen(t) * 8);
		}
		else
		{
			x = 64;
		...
		sprintf(string + Q_strlen(string), "xv %d ", x - ((hnd->cur == i) ? 8 : 0));

		if (hnd->cur == i)
		{
			sprintf(string + Q_strlen(string), "string2 \"\x0d%s\" ", t);
		}
		else
		if (alt)
		{
			sprintf(string + Q_strlen(string), "string2 \"%s\" ", t);
		}
		else
		{
			sprintf(string + Q_strlen(string), "string \"%s\" ", t);
		}
