...
qboolean Add_Ammo(edict_t *ent, gitem_t *item, int count)
{
	...
	if (item->tag == AMMO_FLECHETTES)
	{
		max = ent->client->pers.max_flechettes;
	}
	else
	if (item->tag == AMMO_PROX)
	{
		max = ent->client->pers.max_prox;
	}
	else
	if (item->tag == AMMO_TESLA)
	{
...
qboolean Pickup_Armor(edict_t *ent, edict_t *other)
{
	...
	else
	{
		...
		if (old_armor_index == combat_armor_index)
		{
 ...
 */
void Item_TriggeredSpawn(edict_t *self, edict_t *other /* unused */, edict_t *activator /* unused */)
{
 ...
 * plants the object on the floorf.
 *
 * Items can't be immediately dropped
 * to floorf, because they might be on
...
void SP_xatrix_item(edict_t *self)
{
	...
	if (!strcmp(self->classname, "ammo_trap"))
	{
		spawnClass = "weapon_proxlauncher";
	}
	else
	if (!strcmp(self->classname, "item_quadfire"))
	{
		float chance;
	...
	if (!strcmp(self->classname, "weapon_boomer"))
	{
		spawnClass = "weapon_etf_rifle";
	}
	else
	if (!strcmp(self->classname, "weapon_phalanx"))
	{
