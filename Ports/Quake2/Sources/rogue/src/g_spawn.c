 ...
 *  CreateGroundMonster - this checks the volume and makes sure the floorf under the volume is suitable
 */
...
void widowlegs_think(edict_t *self)
{
	...
	if (level.time > self->wait)
	{
		...
		ThrowWidowGibSized(self, "models/monsters/blackwidow/gib1/tris.md2",
			80 + (int)(random() * 20.0f), GIB_METALLIC, point, 0, true);
	...
	if ((level.time > (self->wait - 0.5f)) && (self->count == 0))
	{
		self->count = 1;
		AngleVectors(self->s.angles, f, r, u);
