...
qboolean blocked_checkplat(edict_t *self, float dist)
{
	...
	if (self->enemy->absmax[2] <= self->absmin[2])
	{
		playerPosition = -1;
	}
	else
...
void hint_path_touch(edict_t *self, edict_t *other, cplane_t *plane /* unused */, csurface_t *surf /* unused */)
{
	edict_t *e, *goal;
	edict_t *next = NULL;
