...
void prox_die(edict_t *self, edict_t *inflictor, edict_t *attacker /* unused */, int damage, vec3_t point)
{
	if (!self || !inflictor)
	{
		return;
...
void Prox_Field_Touch(edict_t *ent, edict_t *other, cplane_t *plane /* unused */, csurface_t *surf /* unused */)
{
	edict_t *prox;
...
void fire_prox(edict_t *self, vec3_t start, vec3_t aimdir, int damage_multiplier, int speed)
{
	...
	VectorMA(prox->velocity, 200 + crandom() * 10.0f, up, prox->velocity);
	VectorMA(prox->velocity, crandom() * 10.0f, right, prox->velocity);
...
void nuke_die(edict_t *self, edict_t *inflictor /* unused */, edict_t *attacker, int damage, vec3_t point)
{
	if (!self || !attacker)
	{
		return;
...
void Nuke_Think(edict_t *ent)
{
	...
	if (level.time >= (ent->wait - NUKE_TIME_TO_LIVE))
	{
		ent->s.frame++;
		...
		if (ent->timestamp <= level.time)
		{
			if ((ent->wait - level.time) <= (NUKE_TIME_TO_LIVE / 2.0f))
			{
				...
				ent->timestamp = level.time + 0.3f;
			}
			else
			{
				gi.sound(ent, CHAN_NO_PHS_ADD + CHAN_VOICE, gi.soundindex("weapons/nukewarn2.wav"), 1, attenuation, 0);
				ent->timestamp = level.time + 0.5f;
			}
		}
	}
	else
...
void nuke_bounce(edict_t *ent, edict_t *other /* unused */, cplane_t *plane /* unused */, csurface_t *surf /* unused */)
{
	if (!ent)
	{
		return;
...
void fire_nuke(edict_t *self, vec3_t start, vec3_t aimdir, int speed)
{
	...
	VectorMA(nuke->velocity, 200 + crandom() * 10.0f, up, nuke->velocity);
	VectorMA(nuke->velocity, crandom() * 10.0f, right, nuke->velocity);
...
void tesla_die(edict_t *self, edict_t *inflictor /* unused */, edict_t *attacker /* unused */, int damage /* unused */, vec3_t point /* unused */)
{
	if (!self)
	{
		return;
...
void tesla_lava(edict_t *ent, edict_t *other /* unused */, cplane_t *plane, csurface_t *surf /* unused */)
{
	vec3_t land_point;
...
void tracker_fly(edict_t *self)
{
	...
	if (VectorCompare(self->enemy->absmin, vec3_origin) ||
