...
void G_SetMovedir(vec3_t angles, vec3_t movedir)
{
	...
	if (VectorCompare(angles, VEC_DOWN))
	{
...
float vectoyaw(vec3_t vec)
{
	...
	{
		yaw = (int)(atan2f(vec[YAW], vec[PITCH]) * 180 / Q_PI);
...
float vectoyaw2(vec3_t vec)
{
	...
	{
		yaw = (atan2f(vec[YAW], vec[PITCH]) * 180 / Q_PI);
...
char* G_CopyString(char *in)
{
	...
	out = gi.TagMalloc(Q_strlen(in) + 1, TAG_LEVEL);
	strcpy(out, in);
	return out;
}
