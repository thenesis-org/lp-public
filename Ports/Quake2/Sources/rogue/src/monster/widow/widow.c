...
WidowBlaster(edict_t *self)
{
	...
	else if ((self->s.frame >= FRAME_fired02a) && (self->s.frame <= FRAME_fired20))
	{
		...
		PredictAim(self->enemy, start, 1000, true, ((random() * 0.1f) - 0.05f), forward, NULL);
...
widow_attack(edict_t *self)
{
	...
	if (self->bad_area)
	{
		if ((random() < 0.1f) || (level.time < self->timestamp))
		{
	...
	if ((realrange(self, self->enemy) > 300) && (!anger) && (random() < 0.5f) &&
		(!blocked))
	{
	...
	if (SELF_SLOTS_LEFT >= 2)
	{
		if ((luck <= 0.40f) && (self->monsterinfo.pausetime + BLASTER_TIME <= level.time))
		{
			self->monsterinfo.currentmove = &widow_move_attack_pre_blaster;
		}
	...
	else
	{
		...
		else if ((luck <= 0.50f) || (level.time + BLASTER_TIME >= self->monsterinfo.pausetime))
		{
			gi.sound(self, CHAN_WEAPON, sound_rail, 1, ATTN_NORM, 0);
			self->monsterinfo.currentmove = &widow_move_attack_pre_rail;
		}
...
widow_attack_blaster(edict_t *self)
{
	...
	self->monsterinfo.pausetime = level.time + 1.0f + (2.0f * random());
...
widow_pain(edict_t *self, edict_t *other /* unused */, float kick, int damage)
{
	...
	{
		if ((skill->value < 3) && (random() < (0.6f - (0.2f * ((float)skill->value)))))
		{
	...
	{
		if ((skill->value < 3) && (random() < (0.75f - (0.1f * ((float)skill->value)))))
		{
...
Widow_CheckAttack(edict_t *self)
{
	...
	if ((random() < 0.8f) && (SELF_SLOTS_LEFT >= 2) &&
