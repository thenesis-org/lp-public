...
widow2_disrupt_reattack(edict_t *self)
{
	...
	if (luck < (0.25f + ((float)(skill->value)) * 0.15f))
	{
...
widow2_attack(edict_t *self)
{
	...
	if (self->bad_area)
	{
		if ((random() < 0.75f) || (level.time < self->monsterinfo.attack_finished))
		{
...
widow2_reattack_beam(edict_t *self)
{
	...
	if (infront(self, self->enemy))
	{
		if (random() <= 0.5f)
		{
			if ((random() < 0.7f) || (SELF_SLOTS_LEFT < 2))
			{
				self->monsterinfo.currentmove = &widow2_move_attack_beam;
			}
			else
...
widow2_pain(edict_t *self, edict_t *other /* unused */, float kick, int damage)
{
	...
	else if (damage < 75)
	{
		...
		if ((skill->value < 3) &&
			(random() < (0.6f - (0.2f * ((float)skill->value)))))
		{
			self->monsterinfo.aiflags &= ~AI_MANUAL_STEERING;
	...
	else
	{
		...
		if ((skill->value < 3) &&
			(random() < (0.75f - (0.1f * ((float)skill->value)))))
		{
			self->monsterinfo.aiflags &= ~AI_MANUAL_STEERING;
			self->monsterinfo.currentmove = &widow2_move_pain;
		}
		...
		vec3_t startpos, qboolean sized, int hitsound, qboolean fade)
{
	...
	if (sized)
	{
		...
		gib->velocity[1] *= 2;
		ClipGibVelocity(gib);
		gib->velocity[2] = max((350 + (random() * 100.0f)), gib->velocity[2]);
		gib->gravity = 0.25;
...
BloodFountain(edict_t *self, int number, vec3_t startpos, int damage)
{
	...
	for (n = 0; n < number; n++)
	{
		...
		WidowVelocityForDamage(damage, vd);
		VectorMA(self->velocity, 1.0f, vd, velocity);
		velocity[0] *= 2;
		velocity[1] *= 2;
	}
}
