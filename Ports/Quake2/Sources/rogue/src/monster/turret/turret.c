...
turret_attack(edict_t *self)
{
	...
	else
	{
		...
		self->monsterinfo.blind_fire_delay += 0.4f + 3.0f + random() * 4.0f;
...
turret_checkattack(edict_t *self)
{
	...
	if (self->enemy->health > 0)
	{
		...
		if (tr.ent != self->enemy)
		{
			...
			if ((self->enemy->solid != SOLID_NOT) || (tr.fraction < 1.0f))
			{
				...
				{
					if ((self->monsterinfo.blindfire) && (self->monsterinfo.blind_fire_delay <= 10.0f))
					{
	...
	{
		chance = 0.10;
		nexttime = (1.8f - (0.2f * skill->value));
	}
	else if (self->spawnflags & SPAWN_BLASTER)
	{
		chance = 0.35;
		nexttime = (1.2f - (0.2f * skill->value));
	}
	else
	{
		chance = 0.50;
		nexttime = (0.8f - (0.1f * skill->value));
	}
