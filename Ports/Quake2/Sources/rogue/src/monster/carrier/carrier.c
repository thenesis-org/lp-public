...
CarrierCoopCheck(edict_t *self)
{
	...
	/* just in case we got a 1.0f from random */
...
CarrierRocket(edict_t *self)
{
	...
	if (self->enemy)
	{
		if (self->enemy->client && (random() < 0.5f))
		{
...
CarrierSpawn(edict_t *self)
{
	...
	mytime = (int)((level.time + 0.1f - self->timestamp) / 0.5f);
...
carrier_spawn_check(edict_t *self)
{
	...
	if (level.time > (self->timestamp + 1.1f))  /* 0.5f seconds per flyer.  this gets three */
	{
		self->monsterinfo.aiflags &= ~AI_MANUAL_STEERING;
...
carrier_reattack_mg(edict_t *self)
{
	...
	if (infront(self, self->enemy))
	{
		if (random() <= 0.5f)
		{
			if ((random() < 0.7f) || (self->monsterinfo.monster_slots <= 2))
			{
				self->monsterinfo.currentmove = &carrier_move_attack_mg;
			}
			else
...
carrier_reattack_gren(edict_t *self)
{
	...
	if (infront(self, self->enemy))
	{
		if (self->timestamp + 1.3f > level.time)  /* four grenades */
		{
			self->monsterinfo.currentmove = &carrier_move_attack_gren;
			return;
		}
