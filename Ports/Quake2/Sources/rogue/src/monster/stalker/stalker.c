...
#define PI 3.14159f
#define RAD2DEG(x) (x * (float)180.0f / (float)PI)
#define DEG2RAD(x) (x * (float)PI / (float)180.0f)
...
stalker_idle_noise(edict_t *self)
{
	...
	gi.sound(self, CHAN_WEAPON, sound_idle, 0.5f, ATTN_IDLE, 0);
}
...
stalker_shoot_attack(edict_t *self)
{
	...
	if (self->groundentity && (random() < 0.33f))
	{
		VectorSubtract(self->enemy->s.origin, self->s.origin, dir);
		dist = VectorLength(dir);

		if ((dist > 256) || (random() < 0.5f))
		{
	...
	if (random() < (0.20f + 0.1f * skill->value))
	{
		dist = VectorLength(dir);
		time = dist / 1000;
...
stalker_shoot_attack2(edict_t *self)
{
	...
	if (random() < (0.4f + (0.1f * (float)skill->value)))
	{
...
stalker_attack_ranged(edict_t *self)
{
	...
	/* circle strafe stuff */
	if (random() > (1.0f - (0.5f / (float)(skill->value))))
	{
...
calcJumpAngle(vec3_t start, vec3_t end, float velocity, vec3_t angles)
{
	...
	distH = sqrtf(dist[0] * dist[0] + dist[1] * dist[1]);
	distV = dist[2];
	...
	if (distV)
	{
		l = sqrtf(distH * distH + distV * distV);
		...
		angles[1] = RAD2DEG((angles[1] - U) / 2.0f);
	}
	else
	{
		l = sqrtf(distH * distH + distV * distV);
		...
		angles[1] = RAD2DEG((angles[1]) / 2.0f);
	}
}
...
stalker_do_pounce(edict_t *self, vec3_t dest)
{
	...
	if (!preferHighJump && (!_isnan(jumpAngles[0])))
	{
		...
		VectorScale(forward, velocity * cosf(DEG2RAD(jumpAngles[0])), self->velocity);
		self->velocity[2] = velocity * sinf(DEG2RAD(jumpAngles[0])) + (0.5f * sv_gravity->value * FRAMETIME);
		return 1;
	}
	...
	if (!_isnan(jumpAngles[1]))
	{
		...
		VectorScale(forward, velocity * cosf(DEG2RAD(jumpAngles[1])), self->velocity);
		self->velocity[2] = velocity * sinf(DEG2RAD(jumpAngles[1])) + (0.5f * sv_gravity->value * FRAMETIME);
		return 1;
	}
...
stalker_jump_wait_land(edict_t *self)
{
	...
	if ((random() < (0.3f + (0.1f * (float)(skill->value)))) &&
		(level.time >= self->monsterinfo.attack_finished))
	{
		self->monsterinfo.attack_finished = level.time + 0.3f;
		stalker_shoot_attack(self);
	}
