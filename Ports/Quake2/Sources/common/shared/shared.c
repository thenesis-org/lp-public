...
void RotatePointAroundVector(vec3_t dst, const vec3_t dir, const vec3_t point, float degrees)
{
	...
	zrot[0][0] = (float)cosf(DEG2RAD(degrees));
	zrot[0][1] = (float)sinf(DEG2RAD(degrees));
	zrot[1][0] = (float)-sinf(DEG2RAD(degrees));
	zrot[1][1] = (float)cosf(DEG2RAD(degrees));
...
void AngleVectors2(vec3_t value1, vec3_t angles)
{
	...
	else
	{
		if (value1[0])
		{
			yaw = (atan2f(value1[1], value1[0]) * 180 / Q_PI);
		}
		else
		if (value1[1] > 0)
		{
		...
		pitch = (atan2f(value1[2], forward) * 180 / Q_PI);
...
{
	char *d = malloc(Q_strlen(s) + 1); // Space for length plus nul
	if (d == NULL)
...
{
	extern int strcasecmp(const char *f1, const char *f2);
...
void Info_RemoveKey(char *s, char *key)
{
	...
	while (1)
	{
		...
		*o = 0;
		...
		if (!strcmp(key, pkey))
		{
			memmove(start, s, Q_strlen(s) + 1); /* remove this part */
			return;
		}
...
void Info_SetValueForKey(char *s, char *key, char *value)
{
	...
	if ((int)(Q_strlen(newi) + Q_strlen(s)) > maxsize)
	{
