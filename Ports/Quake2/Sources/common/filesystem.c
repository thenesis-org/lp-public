...
static char fs_writableGamedir[MAX_OSPATH];
 ...
 */
qboolean FS_CreatePath(char *path)
{
	char *cur; /* Current '/'. */
	char *old; /* Old '/'. */
	...
	while (cur != NULL)
	{
		if ((cur - old) > 1)
		{
			*cur = '\0';
			if (Sys_Mkdir(path))
			{
				Com_Printf("WARNING: cannot create path '%s'.\n", path);
				return true;
			}
			*cur = '/';
		}
...
int FS_FOpenFile(const char *name, fileHandle_t *f, qboolean gamedir_only)
{
	...
	for (search = fs_searchPaths; search; search = search->next)
	{
		if (gamedir_only)
		{
			if (strstr(search->path, FS_WritableGamedir()) == NULL)
			{
				continue;
			}
		}
...
static void FS_AddGameDirectory(const char *dir)
{
	...
	for (i = 0; i < (int)(sizeof(fs_packtypes) / sizeof(fs_packtypes[0])); i++)
	{
...
static void FS_AddHomeAsGameDirectory(const char *dir)
{
	...
	size_t len = snprintf(gdir, sizeof(gdir), "%s%s/", home, dir);
...
static void FS_AddBinaryDirAsGameDirectory(const char * dir)
{
	...
	if ((len > 0) && (len < (int)sizeof(gdir)) && (gdir[len - 1] == '/'))
	{
		gdir[len - 1] = 0;
	}
...
void FS_SetGamedir(char *dir)
{
	...
	FS_AddGameDirectories(dir);

	if (strcmp(dir, BASEDIRNAME) == 0)
	{
		Cvar_FullSet("gamedir", "", CVAR_SERVERINFO | CVAR_NOSET);
		Cvar_FullSet("game", "", CVAR_LATCH | CVAR_SERVERINFO); // It must be last because dir can point on it and it becomes "" after the call of this function.
	}
	else
	{
 ...
 */
qboolean ComparePackFiles(const char *findname, const char *name, unsigned musthave, unsigned canthave, char *output, int size)
{
	qboolean retval;
	char *ptr;
	...
	if ((canthave & SFF_SUBDIR) && (name[Q_strlen(name) - 1] == '/'))
	{
		return false;
	}
...
char** FS_ListFiles2(char *findname, int *numfiles, unsigned musthave, unsigned canthave)
{
	...
	for (search = fs_searchPaths; search != NULL; search = search->next)
	{
		...
		if (tmplist != NULL)
		{
			...
			{
				list[j] = Q_strdup(tmplist[i] + Q_strlen(search->path) + 1);
			}
