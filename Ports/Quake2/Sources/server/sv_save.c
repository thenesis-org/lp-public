...
void SV_WipeSavegame(char *savename)
{
	...
	Com_sprintf(name, sizeof(name), "%s/save/%s/*.sav", FS_WritableGamedir(), savename);
	...
	Sys_FindClose();
	Com_sprintf(name, sizeof(name), "%s/save/%s/*.sv2", FS_WritableGamedir(), savename);
	s = Sys_FindFirst(name, 0, 0);
...
void SV_CopySaveGame(char *src, char *dst)
{
	...
	Com_sprintf(name, sizeof(name), "%s/save/%s/server.ssv", FS_WritableGamedir(), src);
	Com_sprintf(name2, sizeof(name2), "%s/save/%s/server.ssv", FS_WritableGamedir(), dst);
	FS_CreatePath(name2);
	CopyFile(name, name2);

	Com_sprintf(name, sizeof(name), "%s/save/%s/game.ssv", FS_WritableGamedir(), src);
	Com_sprintf(name2, sizeof(name2), "%s/save/%s/game.ssv", FS_WritableGamedir(), dst);
	CopyFile(name, name2);

	Com_sprintf(name, sizeof(name), "%s/save/%s/", FS_WritableGamedir(), src);
	len = Q_strlen(name);
	Com_sprintf(name, sizeof(name), "%s/save/%s/*.sav", FS_WritableGamedir(), src);
	...
	while (found)
	{
		...
		Com_sprintf(name2, sizeof(name2), "%s/save/%s/%s",
			FS_WritableGamedir(), dst, found + len);
		CopyFile(name, name2);
...
void SV_WriteServerFile(qboolean autosave)
{
	...
	Com_sprintf(name, sizeof(name), "%s/save/current/server.ssv", FS_WritableGamedir());
	f = fopen(name, "wb");
	...
	for (var = cvar_vars; var; var = var->next)
	{
		...
		if ((Q_strlen(var->name) >= (int)sizeof(cvarname) - 1) ||
		    (Q_strlen(var->string) >= (int)sizeof(string) - 1))
		{
	...
	/* write game state */
	Com_sprintf(name, sizeof(name), "%s/save/current/game.ssv", FS_WritableGamedir());
