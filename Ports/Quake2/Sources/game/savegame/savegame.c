...
void ReadGame(const char *filename)
{
	...
	if (!strcmp(str_ver, SAVEGAMEVER))
	{
		...
		if (strcmp(str_os, OSTYPE))
		{
			fclose(f);
			gi.error("Savegame from an other os.\n");
		}
		else
		if (strcmp(str_arch, ARCH))
		{
			fclose(f);
