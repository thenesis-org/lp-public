...
void turret_breach_think(edict_t *self)
{
	...
	if (self->move_angles[PITCH] < self->pos2[PITCH])
	{
	...
	if (self->owner)
	{
		...
		angle *= (Q_PI * 2 / 360);
		target[0] = SnapToEights(self->s.origin[0] + cosf(
					angle) * self->owner->move_origin[0]);
		target[1] = SnapToEights(self->s.origin[1] + sinf(
					angle) * self->owner->move_origin[0]);
		...
		target_z = SnapToEights(
				self->s.origin[2] + self->owner->move_origin[0] * tanf(
 ...
 */
void turret_driver_die(edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point /* unused */)
{
	edict_t *ent;
