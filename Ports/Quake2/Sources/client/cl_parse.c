...
void CL_ParseFrame(void)
{
	...
	else
	{
		...
		if (cl.parse_entities - old->parse_entities > MAX_PARSE_ENTITIES -
		    128)
		{
	...
	if (cl.time < cl.frame.servertime - 100)
	{
...
void CL_LoadClientinfo(clientinfo_t *ci, char *s)
{
	...
	else
	{
		...
		*t = 0;
		...
		strcpy(skin_name, s + Q_strlen(model_name) + 1);
...
void CL_ParseConfigString(void)
{
	...
	length = Q_strlen(s);
	if (length > (int)(sizeof(cl.configstrings) - sizeof(cl.configstrings[0]) * i - 1))
	{
	...
	if ((i >= CS_SOUNDS) && (i < CS_SOUNDS + MAX_MODELS))
	{
		if (cl.refresh_prepped)
		{
	...
	if ((i >= CS_IMAGES) && (i < CS_IMAGES + MAX_MODELS))
	{
		if (cl.refresh_prepped)
		{
