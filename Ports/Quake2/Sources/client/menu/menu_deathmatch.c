...
static void DMFlagCallback(void *self)
{
	...
	if (f == &s_instant_powerups_box)
	{
		bit = DF_INSTANT_ITEMS;
	}
	else
	if (f == &s_allow_exit_box)
	{
		bit = DF_ALLOW_EXIT;
	}
	else
	...
	if (f == &s_fixed_fov_box)
	{
		bit = DF_FIXED_FOV;
	}
	else
	if (f == &s_quad_drop_box)
	{
		bit = DF_QUAD_DROP;
	}
	else
	if (Developer_searchpath() == 2)
	{
		...
		if (f == &s_stack_double_box)
		{
			bit = DF_NO_STACK_DOUBLE;
		}
		else
		if (f == &s_no_spheres_box)
		{
			bit = DF_NO_SPHERES;
		}
	}
