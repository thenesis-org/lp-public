...
void Field_Draw(menufield_s *f)
{
	...
	if (n > (int)sizeof(tempbuffer))
	{
		n = (int)sizeof(tempbuffer);
	}
...
qboolean Field_Key(menufield_s *f, int key)
{
	...
	case K_DEL:
		memmove(&f->buffer[f->cursor], &f->buffer[f->cursor + 1],
			Q_strlen(&f->buffer[f->cursor + 1]) + 1);
		break;
...
void Menu_AddItem(menuframework_s *menu, void *item)
{
	...
	{
		menu->nslots = 0;
		//menu->itemPerPageNb = 0;
		//menu->firstItemVisible = 0;
	}
...
void Menu_AdjustCursor(menuframework_s *m, int dir)
{
	...
	qboolean invalid = true;
	int cursor = m->cursor;
	if ((cursor >= 0) && (cursor < m->nitems))
	{
	...
	int itemPerPageNb = m->itemPerPageNb;
	int firstItemVisible = m->firstItemVisible;
	if (itemPerPageNb > 0)
	{
		if ((firstItemVisible > cursor) || (firstItemVisible + (itemPerPageNb - 1) < cursor))
			firstItemVisible = (m->cursor / itemPerPageNb) * itemPerPageNb;
	}
	else
	{
		firstItemVisible = 0;
	}
	m->firstItemVisible = firstItemVisible;
}

static int Menu_GetTotalHeight(menuframework_s *menu)
{
	int ymin = VID_HEIGHT, ymax = 0;
...
{
	float scale = SCR_GetMenuScale();
	int screenHeight = (int)(VID_HEIGHT / scale);
	int menuHeight = Menu_GetTotalHeight(menu);
	int statusHeight = 10;
	menu->y = (screenHeight - menuHeight - statusHeight) / 2;
}

void Menu_CenterWithBanner(menuframework_s *menu, char *bannerName)
{
	float scale = SCR_GetMenuScale();
	int screenHeight = (int)(VID_HEIGHT / scale);

	int bannerWidth, bannerHeight;
	Draw_GetPicSize(&bannerWidth, &bannerHeight, bannerName);
	int bannerY = bannerHeight + 10;

	int menuHeight = Menu_GetTotalHeight(menu);
	int statusHeight = 10;
	menu->y = bannerY + (screenHeight - bannerY - menuHeight - statusHeight) / 2;
}
...
void Menu_Draw(menuframework_s *menu)
{
	...
	if (itemPerPageNb <= 0 || itemPerPageNb > menu->nitems)
		itemPerPageNb = menu->nitems;

	int i0 = menu->firstItemVisible;
	if (i0 < 0)
		i0 = 0;
	int i1 = i0 + itemPerPageNb;
	if (i1 > menu->nitems)
	{
		i1 = menu->nitems;
		//i0 = i1 - itemPerPageNb;
	...
	for (int i = i0; i < i1; i++)
	{
		...
		{
		case MTYPE_FIELD:
			Field_Draw((menufield_s *)item);
			break;
		case MTYPE_SLIDER:
			Slider_Draw((menuslider_s *)item);
			break;
		case MTYPE_LIST:
			MenuList_Draw((menulist_s *)item);
			break;
		case MTYPE_SPINCONTROL:
			SpinControl_Draw((menulist_s *)item);
			break;
		case MTYPE_ACTION:
			Action_Draw((menuaction_s *)item);
			break;
		case MTYPE_SEPARATOR:
			Separator_Draw((menuseparator_s *)item);
			break;
		}
	}
...
void Menu_DrawString(int x, int y, const char *string)
{
	float scale = SCR_GetMenuScale();
	Draw_CharBegin();
	for (int i = 0; i < Q_strlen(string); i++)
	{
...
void Menu_DrawStringR2L(int x, int y, const char *string)
{
	...
	for (int i = 0; i < Q_strlen(string); i++)
	{
		Draw_CharScaled(x - i * 8 * scale, y * scale, string[Q_strlen(string) - i - 1], scale);
	}
	Draw_CharEnd();
}
...
void Menu_DrawStringR2LDark(int x, int y, const char *string)
{
	...
	for (int i = 0; i < Q_strlen(string); i++)
	{
		Draw_CharScaled(x - i * 8 * scale, y * scale, string[Q_strlen(string) - i - 1] + 128, scale);
	}
	Draw_CharEnd();
}
