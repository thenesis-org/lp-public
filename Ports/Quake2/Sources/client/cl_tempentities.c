...
void CL_ParsePlayerBeam(struct model_s *model)
{
	...
	if (model == cl_mod_monster_heatbeam)
	{
...
void CL_ParseTEnt(void)
{
	...
	case TE_BLASTER:         /* blaster hitting wall */
		...
		ex->ent.angles[0] = (float)acos(dir[2]) / Q_PI * 180;

		if (dir[0])
		{
			ex->ent.angles[1] = atan2f(dir[1], dir[0]) / Q_PI * 180;
		}
		else
		if (dir[1] > 0)
		{
...
void CL_AddBeams(void)
{
	...
	for (i = 0, b = cl_beams; i < MAX_BEAMS; i++, b++)
	{
		...
		else
		{
			if (dist[0])
			{
				yaw = (atan2f(dist[1], dist[0]) * 180 / Q_PI);
			}
			else
			if (dist[1] > 0)
			{
			...
			pitch = (atan2f(dist[2], forward) * -180.0f / Q_PI);
		...
		steps = (float)ceilf(d / model_length);
