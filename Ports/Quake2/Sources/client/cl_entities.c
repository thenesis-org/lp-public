		...
		if ((effects & ~EF_ROTATE))
		{
			...
			if (effects & EF_TRACKERTRAIL)
			{
				if (effects & EF_TRACKER)
				{
					...
					intensity = 50 + (500 * ((float)sinf(cl.time / 500.0f) + 1.0f));
...
{
	static const float pi = Q_PI; /* float instead of double */
	 ...
	 * Formula:
	 *
	 * fov = 2.0 * atan(width / height * 3.0 / 4.0 * tanf(fov43 / 2.0))
	 *
