...
void CL_BigTeleportParticles(vec3_t org)
{
	...
	for (i = 0; i < 4096; i++)
	{
		...
		angle = Q_PI * 2 * (randk() & 1023) / 1023.0f;
		dist = (float)(randk() & 31);
		p->org[0] = org[0] + (float)cosf(angle) * dist;
		p->vel[0] = (float)cosf(angle) * (70 + (randk() & 63));
		p->accel[0] = -(float)cosf(angle) * 100;

		p->org[1] = org[1] + (float)sinf(angle) * dist;
		p->vel[1] = (float)sinf(angle) * (70 + (randk() & 63));
		p->accel[1] = -(float)sinf(angle) * 100;
...
void CL_BfgParticles(entity_t *ent)
{
	...
	for (i = 0; i < NUMVERTEXNORMALS; i++)
	{
		...
		p->color = (int)floorf(0xd0 + dist * 7);
		p->colorvel = 0;
		p->alpha = 1.0f - dist;
		p->alphavel = -100;
	}
...
void CL_ForceWall(vec3_t start, vec3_t end, int color8)
{
	...
	while (len > 0)
	{
		...
		if (frandk() > 0.3f)
		{
			...
			p->alpha = 1.0f;
			p->alphavel = -1.0f / (3.0f + frandk() * 0.5f);
			p->color = color8;
...
void CL_Heatbeam(vec3_t start, vec3_t forward)
{
	...
	for (i = start_pt; i < len; i += step)
	{
		...
		for (rot = 0; rot < Q_PI * 2; rot += rstep)
		{
			if (!free_particles)
			{
				return;
			...
			VectorClear(p->accel);
			variance = 0.5f;
			c = (float)cosf(rot) * variance;
			s = (float)sinf(rot) * variance;
...
void CL_ParticleSteamEffect2(cl_sustain_t *self)
{
	...
	for (i = 0; i < self->count; i++)
	{
		...
		for (j = 0; j < 3; j++)
		{
			p->org[j] = self->org[j] + self->magnitude * 0.1f * crandk();
		}
