...
#define TURBSCALE (256.0f / (2 * Q_PI))
...
void R_BoundPoly(int numverts, float *verts, vec3_t mins, vec3_t maxs)
{
	int i, j;
	float *v;
...
void R_SubdividePolygon(int numverts, float *verts)
{
	int i, j, k;
	vec3_t mins, maxs;
	float m;
...
static OpenGLWrapperVertex* R_MakeSkyVec(OpenGLWrapperVertex *vtx, float s, float t, int axis)
{
	vec3_t v, b;
	int j, k;
...
void R_DrawSkyBox()
{
	...
	oglwRotate(r_newrefdef.time * skyrotate, skyaxis[0], skyaxis[1], skyaxis[2]);
	...
	for (i = 0; i < 6; i++)
	{
		...
		oglwBindTexture(0, sky_images[skytexorder[i]]->texnum);
		oglwBegin(GL_TRIANGLES);
		OpenGLWrapperVertex *vtx = oglwAllocateQuad(4);
		vtx = R_MakeSkyVec(vtx, skymins[0][i], skymins[1][i], i);
		vtx = R_MakeSkyVec(vtx, skymins[0][i], skymaxs[1][i], i);
		vtx = R_MakeSkyVec(vtx, skymaxs[0][i], skymaxs[1][i], i);
		vtx = R_MakeSkyVec(vtx, skymaxs[0][i], skymins[1][i], i);
		oglwEnd();
	}
