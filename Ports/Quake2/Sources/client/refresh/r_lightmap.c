...
void LM_UploadBlock(qboolean dynamic)
{
	...
	else
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, GL_LIGHTMAP_FORMAT, BLOCK_WIDTH, BLOCK_HEIGHT, 0, GL_LIGHTMAP_FORMAT, GL_UNSIGNED_BYTE, gl_lms.lightmap_buffer);
...
void LM_CreateSurfaceLightmap(msurface_t *surf)
{
	...
	byte *base = gl_lms.lightmap_buffer + (surf->light_t * BLOCK_WIDTH + surf->light_s) * LIGHTMAP_BYTES;
