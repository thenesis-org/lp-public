...
void NetadrToSockadr(netadr_t *a, struct sockaddr_storage *s)
{
	...
	case NA_IP:
        {
            ((struct sockaddr_in *)s)->sin_family = AF_INET;
            int *ip = (int *)&a->ip;
            *(int *)&((struct sockaddr_in *)s)->sin_addr = *ip;
            ((struct sockaddr_in *)s)->sin_port = a->port;
        }
		break;
	...
	case NA_IP6:
        {
            struct in6_addr *ipv6 = (struct in6_addr *)a->ip;
            if (IN6_IS_ADDR_V4MAPPED(ipv6))
            {
                s->ss_family = AF_INET;
                memcpy(&((struct sockaddr_in *)s)->sin_addr, &ipv6->s6_addr[12], sizeof(struct in_addr));
                ((struct sockaddr_in *)s)->sin_port = a->port;
            }
            else
            {
...
void SockadrToNetadr(struct sockaddr_storage *s, netadr_t *a)
{
	...
	{
        int *ipv4 = (int *) &a->ip;
		*ipv4 = *(int *)&((struct sockaddr_in *)s)->sin_addr;
